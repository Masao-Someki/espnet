...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer, mask=None):
        ...
        if mask is not None:
            outputs, mask = sublayer(self.norm(x), mask)
            return x + self.dropout(outputs), mask
        else:
...
class LocalRNN(nn.Module):
    ...
    def get_K(self, x):
        ...
        zeros = self.zeros.unsqueeze(0).repeat(batch_size, 1, 1).cuda()
...
class RTransformer_Block(nn.Module):
    """
    One Block
    """
    def __init__(self, input_dim, output_dim, rnn_type, ksize, N, h, dropout):
        super(RTransformer_Block, self).__init__()
        self.layers = clones(
    ...
    def forward(self, x, mask):
        print('if x is cuda or not:' + str(x.is_cuda))
        if not x.is_cuda:
            x.cuda()
        n, l, d = x.shape
        for i, layer in enumerate(self.layers):
            x = layer(x)
        x, mask = self.connections[0](x, self.pooling, mask=mask)
