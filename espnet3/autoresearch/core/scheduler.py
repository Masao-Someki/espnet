...
class AutoResearchGraphRuntime:
    ...
    def _call_keypoint_agent(
        ...
        if not base_cmd:
            return None
        cmd = base_cmd + ["resume", trial.codex_thread_id, "-"]
    ...
    def _maybe_check_keypoints(self, trial_id: str) -> None:
        """At keypoint epochs, ask agent whether to early-stop the trial."""
        # Respect config flag
        ...
        # Read total_epochs from resolved training config
        ...
        if total_epochs <= 0:
            return
        keypoint_interval = max(1, round(total_epochs / 10))
        ...
        if current_epoch is None or current_epoch < keypoint_interval:
            return

        target_kp = (current_epoch // keypoint_interval) * keypoint_interval
        ...
        with named_lock(trial_dir, "keypoints"):
            ...
            if kp_file.exists():
                try:
                    checked = json.loads(kp_file.read_text(encoding="utf-8"))
                except Exception:
                    checked = []
            if target_kp in checked:
                return
            ...
            prompt = "\n".join([
                f"Keypoint check for {trial_id} at epoch {current_epoch}/{total_epochs}.",
                "",
            ...
            kp_dir = trial_dir / "keypoints" / f"epoch_{target_kp:06d}"
            kp_dir.mkdir(parents=True, exist_ok=True)

            self.logger.info(
                f"Keypoint check for {trial_id} at epoch {target_kp}/{total_epochs}"
            )
            result = self._call_keypoint_agent(trial, prompt, kp_dir)

            checked.append(target_kp)
            kp_file.write_text(json.dumps(checked), encoding="utf-8")
