...
class ScoreRunner(BaseRunner):
    ...
    def __init__(
        ...
        self._metric_specs: List[MetricEvalSpec] = []
        self._results: Dict[str, Dict[str, Any]] = {}

        for metric_cfg in self.metric_cfgs:
            self._metric_specs.extend(self._expand_metric(metric_cfg))

    def _expand_metric(self, metric_cfg: DictConfig) -> Sequence[MetricEvalSpec]:
        ...
        for test_name in applicable_tests:
            ...
            specs.append(MetricEvalSpec(metric_cfg_copy, inputs, test_name))
